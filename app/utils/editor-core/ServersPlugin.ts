...
class ServersPlugin implements IPluginTempl {
  ...
  renderITextPath(textPaths: Record<'id' | 'path', any>[]) {
    textPaths.forEach((item) => {
      ...
      if (object) {
        fabric.Path.fromObject(item.path, (e: fabric.Path) => {
          object.set('path', e);
        });
      }
    });
  ...
  dragAddItem(item: fabric.Object, event?: DragEvent) {
    if (event) {
      ...
      if (event.clientX < left || event.clientY < top || item.width === undefined) return;
  ...
  _getSaveSvgOption() {
    ...
    let fontFamilyArry = this.canvas
      ...
      .map((item) => (item as fabric.Textbox).fontFamily);
