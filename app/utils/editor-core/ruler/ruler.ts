...
class CanvasRuler {
  ...
  private eventHandler: {
    calcObjectRect: (e?: any) => void;
    clearStatus: () => void;
    canvasMouseDown: (e: fabric.IEvent<MouseEvent>) => void;
    canvasMouseMove: (e: fabric.IEvent<MouseEvent>) => void;
    canvasMouseUp: (e: fabric.IEvent<MouseEvent>) => void;
    render: (e: any) => void;
  } = {
  ...
  public enable() {
    ...
    // 绑定事件
    this.options.canvas.on('after:render', this.eventHandler.calcObjectRect as any);
    this.options.canvas.on('after:render', this.eventHandler.render as any);
    this.options.canvas.on('mouse:down', this.eventHandler.canvasMouseDown as any);
    this.options.canvas.on('mouse:move', this.eventHandler.canvasMouseMove as any);
    this.options.canvas.on('mouse:up', this.eventHandler.canvasMouseUp as any);
    this.options.canvas.on('selection:cleared', this.eventHandler.clearStatus as any);
  ...
  public disable() {
    // 解除事件
    this.options.canvas.off('after:render', this.eventHandler.calcObjectRect as any);
    this.options.canvas.off('after:render', this.eventHandler.render as any);
    ...
    this.options.canvas.off('selection:cleared', this.eventHandler.clearStatus as any);
  ...
  public render() {
    ...
    if (!vpt) return;
    const startCalibration = this.startCalibration;
    // 绘制尺子
    this.draw({
      isHorizontal: true,
      rulerLength: this.getSize().width,
      startCalibration: startCalibration?.x !== undefined ? startCalibration!.x : -(vpt[4] / vpt[0]),
    });
    this.draw({
      isHorizontal: false,
      rulerLength: this.getSize().height,
      startCalibration: startCalibration?.y !== undefined ? startCalibration!.y : -(vpt[5] / vpt[3]),
    });
    // 绘制左上角的遮罩
    drawMask(this.ctx, {
      isHorizontal: true,
