...
watch(
  ...
  (article) => {
    ...
    form.content = article?.content || '<p>从这里开始写正文。</p>'
...
<template>
  <div class="space-y-6">
    <UiCard class="p-6">
      <div class="grid gap-5 md:grid-cols-2">
        <label class="block">
          ...
          <UiInput v-model="form.title" placeholder="输入文章标题" />
        </label>
        ...
        <label class="block">
          ...
          <UiInput v-model="form.slug" placeholder="my-first-post" />
        </label>
      </div>
      ...
      <div class="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        ...
        <UiCard class="p-5">
          ...
          <label class="mt-4 flex items-start gap-3">
            <UiCheckbox v-model="form.published" class="mt-0.5" />
            <span>
      ...
      <label class="mt-5 block">
        ...
        <UiInput v-model="form.coverImage" placeholder="https://example.com/cover.jpg" />
      </label>
    ...
    <UiCard class="p-6">
      ...
      <TinyMceEditor v-model="form.content" :disabled="props.loading" />
    </UiCard>

    <div class="flex flex-wrap items-center gap-3">
      <UiButton :disabled="props.loading" @click="handleSubmit">
